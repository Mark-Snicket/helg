...
public class HelgeriteModel extends EntityModel<HelgeriteRenderState> {
    ...
    public HelgeriteModel(ModelPart root) {
        ...
        this.idleAnimation = HelgeriteAnimations.IDLE.bake(root);
    }
    ...
    public void setupAnim(HelgeriteRenderState state) {
        ...
        this.idleAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2.0f, 2.5f);
    }

    private void applyHeadRotation(float headYaw) {
        headYaw = Mth.clamp(headYaw, -30.0f, 30.0f);

        this.helgerite.yRot = headYaw * ((float)Math.PI / 180F);
    }
}